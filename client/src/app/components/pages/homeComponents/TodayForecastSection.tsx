            ...
            {isLoadingDataThreeHoursForecast || isFetchingListThreeHoursForecast || !dataListThreeHoursForecast ? (
                ...
                <>
                    <div className="w-full max-w-full grid grid-cols-3 px-4 gap-8 md:gap-0 md:grid-cols-6 md:px-0 ">
                        {(dataListThreeHoursForecast as any)?.list?.slice(0, 6).map((item: ForecastItem, i: number) => (
                            <div
                                className={`flex flex-col gap-4 justify-center items-center ${i !== (dataListThreeHoursForecast as any)?.list?.slice(0, 6).length - 1 ? 'border-b p-2 md:border-r md:border-b-0' : ''}`}
                                key={i}
                            >
